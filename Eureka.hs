...
The number 89 is the first integer with more
than one digit that fulfills the property partially
...
and outputs a list of the sorted numbers in the range that
...
sumDigPow a b = [ x | x <- [a..b-1], x == (g $ f x)]
          where f xs = map (\x -> read [x] :: Int) (show xs)
                g xs = sum $ map (\(x, y) -> x^y) (zip xs [1..(length xs)])
